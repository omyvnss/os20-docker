# Standalone single-email verification for OS20 Lead Service.
# DNS MX resolution + optional SMTP (port 25) RCPT check.
#
# SMTP RCPT probing can flag the sender IP with some providers --
# keep it best-effort with tight timeouts and treat SMTP failure as
# `unknown`, never as `deliverable`. Add a relay + rate limit before
# serving many concurrent probes.
import logging
import smtplib
import socket
from typing import Optional

import dns.resolver

from leadgen.netguard import allow_private_targets, is_public_address

logger = logging.getLogger(__name__)

# Kept under the server's 12s per-email budget (DNS 3s + SMTP 4s).
_SMTP_TIMEOUT = 4
_DNS_LIFETIME = 3


def mx_hosts(domain: str) -> list[str]:
    try:
        recs = dns.resolver.resolve(domain, "MX", lifetime=_DNS_LIFETIME)
    except Exception:
        return []
    return [
        str(r.exchange).rstrip(".")
        for r in sorted(recs, key=lambda r: r.preference)
    ]


def public_mx_address(host: str) -> Optional[str]:
    try:
        infos = socket.getaddrinfo(host, 25, proto=socket.IPPROTO_TCP)
    except (socket.gaierror, UnicodeError):
        return None
    addresses = [str(info[4][0]) for info in sorted(infos, key=lambda info: info[0] != socket.AF_INET)]
    if not addresses:
        return None
    if not allow_private_targets() and not all(is_public_address(a) for a in addresses):
        return None
    return addresses[0]


def verify_email(email: str, smtp_timeout: float = _SMTP_TIMEOUT) -> dict:
    email = (email or "").strip().lower()
    domain = email.rsplit("@", 1)[1] if "@" in email else ""

    result = {
        "email": email,
        "domain": domain,
        "has_mx": False,
        "mx_hosts": [],
        "deliverable": None,  # True / False / None(unknown)
        "detail": "invalid-email",
        # False when the SMTP connection itself failed (port 25 blocked, timeout).
        "smtp_connected": None,
    }
    if not domain:
        return result

    hosts = mx_hosts(domain)
    result["mx_hosts"] = hosts
    if not hosts:
        result["detail"] = "no-mx"
        result["deliverable"] = False
        return result
    result["has_mx"] = True
    result["detail"] = "mx-present"

    mx_address = public_mx_address(hosts[0])
    if mx_address is None:
        result["smtp_connected"] = False
        result["detail"] = "smtp-unavailable: mx-not-public"
        return result

    try:
        smtp = smtplib.SMTP(timeout=smtp_timeout)
    except OSError as exc:
        result["smtp_connected"] = False
        result["detail"] = f"smtp-unavailable: {type(exc).__name__}"
        return result

    try:
        try:
            smtp.connect(mx_address, 25)
        except (smtplib.SMTPConnectError, socket.timeout, OSError) as exc:
            smtp.close()
            result["smtp_connected"] = False
            result["detail"] = f"smtp-unavailable: {type(exc).__name__}"
            return result
        result["smtp_connected"] = True
        with smtp:
            smtp.helo("os20-leadgen.local")
            smtp.mail("verify@os20-leadgen.local")
            code, _ = smtp.rcpt(email)
            if code == 250:
                result["deliverable"] = True
                result["detail"] = "rcpt-accepted"
                # catch-catch-all
                fake = f"zzznonexistent{email.split('@')[0]}@{domain}"
                fake_code, _ = smtp.rcpt(fake)
                if fake_code == 250:
                    result["detail"] = "catch-all"
                    result["deliverable"] = None
            elif code in (550, 551, 553):
                result["deliverable"] = False
                result["detail"] = "rcpt-rejected"
            else:
                result["detail"] = f"rcpt-code-{code}"
    except (
        smtplib.SMTPServerDisconnected,
        smtplib.SMTPConnectError,
        socket.timeout,
        ConnectionRefusedError,
        OSError,
    ) as exc:
        logger.debug("SMTP verify failed for %s: %s", email, exc)
        result["detail"] = f"smtp-unavailable: {type(exc).__name__}"

    return result