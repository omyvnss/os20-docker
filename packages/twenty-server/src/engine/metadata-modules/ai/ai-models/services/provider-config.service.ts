...
export class ProviderConfigService {
  ...
  // Catalog providers without an API key template (e.g. Ollama) need no key.
  isKeylessCatalogProvider(providerName: string): boolean {
    const entry =
      this.defaultAiCatalogService.getDefaultAiCatalog()[providerName];

    return !!entry && !entry.apiKey && !entry.accessKeyId;
  }
