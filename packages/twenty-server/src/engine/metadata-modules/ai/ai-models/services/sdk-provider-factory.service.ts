...
export class SdkProviderFactoryService {
  ...
  // Build a provider instance without caching it, so per-workspace BYOK keys
  // never collide with the shared env-key instances (which are keyed by
  // provider name). ponytail: uncached by design for BYOK; cache only if
  // per-workspace instances start getting hot.
  createProviderWithKey(config: AiProviderConfig): AiSdkProviderInstance {
