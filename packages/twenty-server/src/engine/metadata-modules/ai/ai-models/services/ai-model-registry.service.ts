...
export class AiModelRegistryService {
  ...
  private ensureFresh(): void {
    const byokKeys = this.apiKeyService.getInstanceKeys();
    const configHash = `${this.configGroupHashService.computeHash(
      ConfigVariablesGroup.LLM,
    )}:${createHash('sha256')
      .update(JSON.stringify([...byokKeys].sort()))
      .digest('hex')}`;
    ...
    this.buildModelRegistry(byokKeys);
    this.currentConfigHash = configHash;
  }

  private buildModelRegistry(byokKeys: Map<string, string>): void {
    ...
    this.registerModelsFromProviders(providers, byokKeys);
  }

  private registerModelsFromProviders(
    ...
    for (const [providerKey, config] of Object.entries(providers)) {
      ...
      const byokKey = byokKeys.get(providerKey);
      const effectiveConfig =
        !config.apiKey && !config.accessKeyId && byokKey
          ? { ...config, apiKey: byokKey }
          : config;

      const sdkInstance = isProviderConfigured(effectiveConfig)
        ? this.sdkProviderFactory.createProvider(providerKey, effectiveConfig)
        : undefined;
  ...
  private getFirstAvailableModelFromList(
    ...
    for (const modelId of modelIds) {
      ...
      if (model && this.hasProviderKey(model)) {
        return model;
      }
    }
  ...
  // Every catalog model is registered so users can pick one before adding its
  // key. Auto-select must only land on providers that can actually answer:
  // a server env key, a key saved in Settings, or a keyless local provider.
  private hasProviderKey(model: RegisteredAiModel): boolean {
    if (!model.providerName) {
      return false;
    }

    const config =
      this.providerConfigService.getResolvedProviders()[model.providerName];

    return Boolean(
      config?.apiKey ||
      config?.accessKeyId ||
      this.apiKeyService.getInstanceKeys().has(model.providerName),
    );
  }

  private canServeAutoSelect(model: RegisteredAiModel): boolean {
    return (
      this.hasProviderKey(model) ||
      (!!model.providerName &&
        this.providerConfigService.isKeylessCatalogProvider(model.providerName))
    );
  }
  ...
  isModelUsable(modelId: string): boolean {
    ...
    return !!model && this.canServeAutoSelect(model);
  }
  ...
  private getDefaultModelForRole(role: AiModelRole): RegisteredAiModel {
    ...
    if (!model) {
      ...
      model =
        availableModels.find((candidate) => this.hasProviderKey(candidate)) ??
        availableModels.find((candidate) => this.canServeAutoSelect(candidate));
    }
  ...
  validateModelAvailability(
    ...
    // BYOK: any configured (admin-allowed) model is selectable. Users bring
    // their own provider key and pick any catalog model; we do not hide models
    // behind the workspace enabled/recommended list. Auto-select resolves to the
    // first model whose provider can answer, so it only needs one of those.
    const isAvailable = isAutoSelectModelId(modelId)
      ? workspaceHasEnabledModels(availabilitySettings, recommendedModelIds) ||
        this.getAvailableModels().some((model) =>
          this.canServeAutoSelect(model),
        )
      : this.getModel(modelId) !== undefined;
  ...
  async resolveModelForAgent(
    ...
    return this.applyWorkspaceApiKey(registeredModel, workspaceId);
  }

  // Override the model's API key with the workspace's own key (pasted in the
  // AI Providers settings page) when one exists, so a BYOK user's key is used
  // instead of the server-side env key. Falls back to the baked-in model.
  // merges per-workspace keys at resolve time; recreate-on-each-call
  // is fine for us but could be memoized if key lookups become hot.
  private async applyWorkspaceApiKey(
    registeredModel: RegisteredAiModel,
    workspaceId: string,
  ): Promise<RegisteredAiModel> {
    const providerInfo = this.providerModelDefCache.get(
      registeredModel.modelId,
    );
    ...
    const workspaceKey = await this.apiKeyService.getKey(
      workspaceId,
      providerInfo.providerName as ProviderId,
    );
    ...
    const providers = this.providerConfigService.getResolvedProviders();
    const providerConfig = providers[providerInfo.providerName];
    ...
    try {
      const sdkInstance = this.sdkProviderFactory.createProviderWithKey({
        ...providerConfig,
        apiKey: workspaceKey,
      });
      ...
      return {
        ...registeredModel,
        model: sdkInstance.createModel(providerInfo.modelDef.name),
      };
    } catch (error) {
      this.logger.warn(
        `Failed to apply workspace API key for ${registeredModel.modelId}: ${error}`,
      );
      return registeredModel;
    }
  }
