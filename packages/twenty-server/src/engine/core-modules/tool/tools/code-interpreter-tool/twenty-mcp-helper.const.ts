...
export const TWENTY_MCP_HELPER = `# Auto-injected OS20 MCP helper - provides access to OS20 tools
import os
import json
...
class OS20MCP:
    """Helper for calling OS20 tools from sandboxed code.
    ...
    def call_tool(self, name: str, arguments: dict = None):
        ...
        Example:
            companies = os20_bridge.call_tool('find_many_companies', {'limit': 5})
            # companies == {'records': [...], 'count': '5'}
        """
        if not self._available:
            raise RuntimeError('OS20 MCP bridge not available. Missing requests library or credentials.')
    ...
    def bulk_upsert(self, plural: str, records: list, batch_size: int = 200):
        ...
        Example:
            summary = os20_bridge.bulk_upsert('people', people_rows)
        """
    ...
    def lookup_by(self, plural: str, field: str, values: list, select: list = None):
        ...
        Example:
            company_ids = os20_bridge.lookup_by('companies', 'name', ['Acme', 'Globex'])
...
# --------------------------------------------------------------------------
# \`os20_bridge\` is a pre-built instance of the OS20MCP class above. It is
# already bound in this module scope — DO NOT \`import os20_bridge\`. There is
# no Python package by that name. Just use it directly, e.g.:
#     companies = os20_bridge.call_tool('find_many_companies', {'limit': 10})
# --------------------------------------------------------------------------
os20_bridge = OS20MCP()
`;
