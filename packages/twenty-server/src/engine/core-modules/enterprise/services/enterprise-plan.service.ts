...
export class EnterprisePlanService implements OnModuleInit {
  ...
  private async loadValidityToken(): Promise<void> {
    try {
      ...
      // In development, fall back to the built-in dev token so enterprise
      // features work for free without an external key.
      ...
      const finalTokenValue =
        tokenValue ??
        (isDevelopment ? ENTERPRISE_DEV_VALIDITY_TOKEN : undefined);
      ...
      const payload = this.verifyJwt<EnterpriseValidityPayload>(finalTokenValue);
