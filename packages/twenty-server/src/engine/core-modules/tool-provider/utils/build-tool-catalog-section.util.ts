...
const getCategoryLabel = (category: ToolCategory): string => {
  switch (category) {
    ...
    case ToolCategory.WEB_AGENT:
      return 'Web Agent Tools (web scraping, lead fetching)';
    case ToolCategory.WEB_SEARCH:
      return 'Web Search Tools (search the internet with your API keys)';
    case ToolCategory.WEB_SCRAPER:
      return 'Web Scrape Tools (extract emails & phone numbers from pages)';
    case ToolCategory.LEAD_GENERATION:
      return 'Lead Generation Tools (find companies and save them to the CRM)';
    default:
