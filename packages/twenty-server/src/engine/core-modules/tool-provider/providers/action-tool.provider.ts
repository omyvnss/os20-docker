...
// Web pages the agent reads can contain instructions, so by default it cannot
// make arbitrary HTTP requests, send email on its own or run code. Emails are
// drafted and the user sends them through outreach, which applies the daily
// limit and the guessed-address check.
export const UNSAFE_AI_TOOL_IDS = new Set([
  'http_request',
  'send_email',
  'code_interpreter',
]);

export const areUnsafeAiToolsEnabled = (): boolean =>
...
export class ActionToolProvider implements ToolProvider {
  ...
  async generateDescriptors(
    ...
    if (hasHttpPermission && areUnsafeAiToolsEnabled()) {
      descriptors.push(
    ...
    if (hasCodeInterpreterPermission && areUnsafeAiToolsEnabled()) {
      descriptors.push(
        this.buildDescriptor(
          'code_interpreter',
  ...
  async executeStaticTool(
    ...
    if (UNSAFE_AI_TOOL_IDS.has(toolName) && !areUnsafeAiToolsEnabled()) {
      throw new Error(
        `The ${toolName} tool is turned off in OS20. Draft the email or ask the user to do this step.`,
      );
    }
