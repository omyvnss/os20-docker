...
export class SecureHttpClientService {
  ...
  // The axios-backed fetch ignores init.signal, so pass timeouts via config.
  createSsrfSafeFetch(
    config?: SecureHttpClientConfig,
  ...
  async getValidatedHost(hostnameOrUrl: string): Promise<string> {
    ...
    return resolveAndValidateHostname(
      hostnameOrUrl,
      undefined,
      this.areEmailPrivateHostsAllowed(),
    );
  }

  // skips private-IP rejection for email hosts only when the self-hosted
  // operator opts in; webhook/SSRF paths don't use getValidatedHost so they stay safe.
  private areEmailPrivateHostsAllowed(): boolean {
    return process.env.ALLOW_PRIVATE_EMAIL_HOSTS === 'true';
  }
