...
<Warning>
Docker Compose is for production hosting or self-hosting. For contributing, see [Local Setup](/developers/contribute/capabilities/local-setup).
</Warning>

## Overview

This guide provides step-by-step instructions to install and configure OS20 using Docker Compose. The aim is to make the process straightforward and prevent common pitfalls.

**Important:** Only modify settings explicitly mentioned in this guide. Altering other configurations may lead to issues.

See [Setup Environment Variables](/developers/self-host/capabilities/setup) for advanced configuration. All environment variables must be declared in the `docker-compose.yml` file at the server and/or worker level.

## System Requirements

- RAM: At least 2GB. Insufficient memory can cause processes to crash.
- Docker & Docker Compose: Installed and up-to-date.

## Option 1: OS20 CLI (recommended)
...
The CLI clones the repo, pulls images, starts the stack, and opens the app in your browser.

Other commands: `os20 stop`, `os20 status`, `os20 logs -f`, `os20 update`, `os20 reset`.
...
### Step 2: Set Up the Environment File .env File**
