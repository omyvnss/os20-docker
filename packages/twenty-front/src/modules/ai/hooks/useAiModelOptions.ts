...
export const useAiModelOptions = ({
  ...
  const autoSelectModelId = isWorkspaceSetupChat
    ? AUTO_SELECT_FAST_MODEL_ID
    : AUTO_SELECT_SMART_MODEL_ID;

  const workspaceDefaultModelId =
    (isWorkspaceSetupChat
      ? currentWorkspace?.fastModel
      : currentWorkspace?.smartModel) ?? autoSelectModelId;

  const findAvailableModel = (modelId: string) =>
    aiModels.find(
      (model) => model.modelId === modelId && model.isAvailable === true,
    );

  const workspaceDefaultModel =
    findAvailableModel(workspaceDefaultModelId) ??
    findAvailableModel(autoSelectModelId);
