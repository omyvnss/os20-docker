...
const SettingsAIProviders = lazy(() =>
  import('~/pages/settings/ai-providers/SettingsAIProviders').then(
    (module) => ({
...
const LeadGeneration = lazy(() =>
  import('~/pages/lead-generation/LeadGeneration').then((module) => ({
    default: module.LeadGeneration,
  })),
);

const WebAgents = lazy(() =>
  import('~/pages/settings/web-agents/WebAgentsSettings').then((module) => ({
    default: module.WebAgentsSettings,
  })),
);

const Os20Setup = lazy(() =>
  import('~/pages/os20-setup/Os20Setup').then((module) => ({
    default: module.Os20Setup,
  })),
);

const WebSearchApis = lazy(() =>
  import('~/pages/settings/web-search-apis/WebSearchApisSettings').then(
    (module) => ({
      default: module.WebSearchApisSettings,
    }),
  ),
);

const SettingsEmailSending = lazy(() =>
  import('~/pages/settings/email-sending/SettingsEmailSending').then(
    (module) => ({
      default: module.SettingsEmailSending,
    }),
  ),
);
...
export const SettingsRoutes = ({ isAdminPageEnabled }: SettingsRoutesProps) => (
  <Suspense fallback={<SettingsSkeletonLoader />}>
    <Routes>
      ...
      <Route
        path={SettingsPath.LeadGeneration}
        element={
          <SettingsProtectedRouteWrapper
            settingsPermission={PermissionFlagType.WORKSPACE}
          >
      ...
      {/* OS20 first-run setup, reached from SignInUp (OS20_SETUP_PATH). */}
      <Route path="setup" element={<Os20Setup />} />
      ...
      <Route
        path={SettingsPath.EmailSending}
        element={
          <SettingsProtectedRouteWrapper
            settingsPermission={PermissionFlagType.WORKSPACE}
          >
            <SettingsEmailSending />
          </SettingsProtectedRouteWrapper>
        }
      />
    </Routes>
