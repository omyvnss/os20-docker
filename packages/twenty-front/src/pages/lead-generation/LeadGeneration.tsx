...
export const LeadGeneration = () => {
  ...
  useEffect(() => {
    authFetch('/lead-generation/leads')
  ...
  const handleSearch = async () => {
    setLoading(true);
    try {
      const response = await authFetch('/lead-generation/find', {
        method: 'POST',
      ...
      if (!response.ok) {
        enqueueErrorSnackBar({
          message: data?.message ?? t`Lead search failed. Try again.`,
        });
        return;
      }
      ...
      if (!data.leads?.length) {
        enqueueErrorSnackBar({
          message:
            data.hints?.[0] ??
            t`No companies found. Try broader keywords or a different location.`,
        });
      }
    } catch {
      enqueueErrorSnackBar({
        message: t`Can't reach the OS20 server. Check that it's running.`,
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleGenerateOutreach = async (lead: Lead) => {
    ...
    try {
      const response = await authFetch('/lead-generation/outreach', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          lead,
      ...
      if (!response.ok) {
        enqueueErrorSnackBar({
          message: data?.message ?? t`Couldn't generate outreach. Try again.`,
        });
        return;
      }
        ...
        >
          <button
            ...
            style={{
              ...
              cursor: 'pointer',
              background:
                mode === 'ai' ? 'var(--background-primary)' : 'transparent',
              ...
              >
                <div>
                  ...
                  <select
                    value={icp.industry}
                    onChange={(e) =>
                      setIcp({ ...icp, industry: e.target.value })
                    }
                    style={{
                      width: '100%',
                      padding: '10px 12px',
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setIcp({ ...icp, companySize: e.target.value })
                    }
                    style={{
                      width: '100%',
                      padding: '10px 12px',
                ...
                <div>
                  ...
                  <input
                    ...
                    value={icp.location}
                    onChange={(e) =>
                      setIcp({ ...icp, location: e.target.value })
                    }
                    style={{
                      width: '100%',
                      padding: '10px 12px',
                ...
                <div>
                  ...
                  <input
                    ...
                    value={icp.keywords}
                    onChange={(e) =>
                      setIcp({ ...icp, keywords: e.target.value })
                    }
                    style={{
                      width: '100%',
                      padding: '10px 12px',
                  ...
                  {leads.map((lead) => (
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <Button
                          title={
                            generatingOutreach && selectedLead?.id === lead.id
                              ? 'Generating...'
                              : 'Generate Outreach'
                          }
                          variant="secondary"
                    ...
                    <Button
                      ...
                      onClick={() => {
                        const subject = encodeURIComponent(
                          `Partnership Opportunity - ${selectedLead.company}`,
                        );
