...
// Sources often link to the listing platform, not the company. Company
// domainName is unique, so saving those would collide on the second import.
...
const isCompanyWebsite = (url: string) => {
  try {
...
export const LeadSourcesPanel = () => {
  ...
  useEffect(() => {
    authFetch('/lead-generation/sources')
  ...
  }, [authFetch, enqueueErrorSnackBar, t]);
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await authFetch('/lead-generation/sources-search', {
        method: 'POST',
      ...
      enqueueErrorSnackBar({
        message: t`Search failed. Check your connection and retry.`,
      });
    } finally {
      setSearching(false);
    }
        ...
        const companyResult = await apolloClient.mutate<{
          ...
          variables: {
            data: {
              ...
              ...(candidate.website && isCompanyWebsite(candidate.website)
                ? {
                    domainName: {
          ...
          await apolloClient.mutate({
            ...
            variables: {
              data: {
                name: {
                  firstName:
                    nameParts.length > 1 ? nameParts[0] : candidate.companyName,
                  lastName:
