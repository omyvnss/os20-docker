...
export const WebAgentsSettings = () => {
  ...
  const load = async () => {
    const response = await authFetch(`${base}/web-agents`);
  ...
  const handleAdd = async () => {
    ...
    setAdding(true);
    try {
      await authFetch(`${base}/web-agents`, {
        method: 'POST',
  ...
  const handleDelete = async (id: string) => {
    await authFetch(`${base}/web-agents/${id}`, { method: 'DELETE' });

    await load();
  };

  const handleToggleEnabled = async (agent: WebAgent) => {
    await authFetch(`${base}/web-agents/${agent.id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  const handleTest = async (id: string) => {
    ...
    try {
      const response = await authFetch(`${base}/web-agents/${id}/test`, {
        method: 'POST',
      });
