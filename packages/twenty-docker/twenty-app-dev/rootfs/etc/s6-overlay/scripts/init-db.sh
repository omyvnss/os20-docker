...
has_schema=$(PGPASSWORD=twenty psql -h localhost -U twenty -d default -tAc \
  "SELECT EXISTS (SELECT 1 FROM information_schema.schemata WHERE schema_name = 'core')" 2>/dev/null || true)

if [ "$has_schema" = "f" ]; then
  step_start "Running initial database setup and migrations"
  if ! yarn database:init:prod; then
    echo "Warning: database:init:prod failed, but continuing startup (the app DB may already be initialized)."
  fi
  step_done
fi
...
has_workspace=$(PGPASSWORD=twenty psql -h localhost -U twenty -d default -tAc \
  "SELECT EXISTS (SELECT 1 FROM core.workspace WHERE id = '20202020-1c25-4d02-bf25-6aeccf7ea419')" 2>/dev/null || true)
